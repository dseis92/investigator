...
export default async function MatterPilotOperationsPage({ searchParams }: { searchParams?: Promise<{ calendar?: string | string[] }> }) {
  ...
  return <OperationsCenter
    ...
    aiRuns={(aiRuns ?? []).map((run) => ({ id: run.id, matterId: run.matter_id, matter: matterNames.get(run.matter_id) ?? "Matter", runType: run.run_type as "matter_brief" | "evidence_summary" | "contradiction_scan" | "timeline_gap_scan" | "missing_document_scan" | "deposition_questions", status: run.status, model: run.model, output: (run.output ?? {}) as { headline?: string; summary?: string; priorities?: string[]; risks?: string[]; questions?: string[]; citedRecords?: string[] }, createdAt: run.created_at }))}
